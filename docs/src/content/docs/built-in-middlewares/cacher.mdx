...
The Cacher Middleware is using the _Stale While Revalidating_ pattern by redispatching the `query` to the bus behind the scene to refresh the cache.
...
3. If the data is _stale_, the middleware will redispatch the `query` to the bus in a fresh new _Envelope_. The `query` will then go through the whole chain of middleware(s) and handler(s), and the result will be cached again.

   The Cacher Middleware knows to avoid the cache lookup thanks to the `ReprocessedStamp` added by the _Envelope_ when the `query` is redispatched.   
</Steps>
...
<Aside title="1 revalidate at a time" type="tip">
The middleware will not revalidate the cache if the `query` is already in the process of being revalidated.
</Aside>
